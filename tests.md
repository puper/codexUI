...
### Provider dropdown: Codex and Custom endpoint only

#### Feature/Change Name
Provider selection only exposes Codex and Custom endpoint. OpenRouter and OpenCode Zen are configured as Custom endpoint URLs, not standalone built-in providers.

#### Prerequisites/Setup
1. Dev server running (`pnpm run dev`).
2. Open Settings panel from the sidebar.
3. For live provider checks, have an API key for the target custom endpoint.

#### Step-by-Step Actions
1. In light theme, open Settings and locate the Provider dropdown.
2. Verify the dropdown contains exactly `Codex` and `Custom endpoint`.
3. Confirm there are no standalone `OpenRouter`, `OpenCode Zen`, free/community-key, rotate-key, or built-in provider key controls.
4. Select `Custom endpoint`.
5. Enter an OpenAI-compatible base URL such as `https://openrouter.ai/api/v1` or `https://opencode.ai/zen/v1`.
6. Enter the provider API key, select the required API format (`Responses` or `Completions`), and click Save.
7. Verify the model list is loaded from the custom endpoint and provider state survives a page refresh.
8. Switch Provider back to `Codex` and confirm custom endpoint fields are hidden and standard Codex behavior resumes.
9. Switch to dark theme and repeat steps 1-3 to verify the same provider list and readable controls.

#### Expected Results
- Provider dropdown exposes only `Codex` and `Custom endpoint` in both light and dark themes.
- OpenRouter and OpenCode Zen work only when entered as custom endpoint base URLs with user-provided keys.
- No bundled OpenRouter community keys, free-mode UI, OpenRouter proxy route, or OpenCode Zen proxy route is used.
- `~/.codex/config.toml` is not modified; custom provider state remains isolated in `~/.codex/webui-free-mode.json`.

#### Rollback/Cleanup
- Switch Provider back to `Codex`.
- Delete `~/.codex/webui-free-mode.json` if a manual custom endpoint test should be cleared.

### Custom endpoint provider isolation

#### Feature/Change Name
Custom endpoint mode isolates external-provider models from Codex models and preserves the selected API format.

#### Prerequisites/Setup
1. Dev server running (`pnpm run dev`).
2. A reachable OpenAI-compatible endpoint and API key.

#### Step-by-Step Actions
1. Open Settings and choose `Custom endpoint`.
2. Enter a custom base URL and API key.
3. Select `Responses`, click Save, refresh the page, and confirm `Responses` remains selected.
4. Select `Completions`, click Save, refresh the page, and confirm `Completions` remains selected.
5. Open the model selector and verify it shows models from the custom endpoint.
6. Switch back to `Codex` and open the model selector again.

#### Expected Results
- Custom endpoint settings persist across refresh.
- Custom endpoint models do not appear after switching back to Codex.
- Codex models do not leak into the custom endpoint model list while custom endpoint mode is active.
